...
export const action = async ({ request }) => {
  ...
  const plan = await budgetPlanning(NumDays, filtereHotelList.data, FilterTaxiList.data);
...
export default function BudgetForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (response.status === 200) {
        localStorage.setItem('budgetPlan', response.data)
        console.log(localStorage.getItem('budgetPlan'))
    ...
    <>
      <div style={{
        ...
        <div style={{ marginLeft: "700px" }}>
          <br /><br />
          <label style={{ fontSize: "20px" }}><h3>Plan on Your Budget </h3></label>
        </div>
